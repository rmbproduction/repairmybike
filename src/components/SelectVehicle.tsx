...
const MultiStepVehicleSelector: React.FC = () => {
  ...
  useEffect(() => {
    const fetchVehicleTypes = async () => {
      try {
        setLoading(true);
        setError(null);
        const url = import.meta.env.VITE_VEHICLE_TYPES_URL;
        if (!url) {
  ...
  const handleSelectVehicleType = async (vehicleType: VehicleType) => {
    ...
    try {
      setLoading(true);
      setError(null);
      const url = import.meta.env.VITE_MANUFACTURERS_URL;
      if (!url) {
  ...
  const handleSelectManufacturer = async (manufacturer: Manufacturer) => {
    ...
    try {
      setLoading(true);
      setError(null);
      const url = import.meta.env.VITE_VEHICLE_MODELS_URL;
      if (!url) {
